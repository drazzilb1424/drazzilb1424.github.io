...
const uploadBox = upload.closest('.slab-preview-upload');
...
function loadFile(file) {
   ...
   reader.onload = () => {
      ...
      image.onload = () => {
         ...
         uploadBox.querySelector('span').textContent = 'Image loaded successfully';
         render();
      };
...
uploadBox.addEventListener('dragover', event => {
   ...
   uploadBox.style.borderColor = guardColor.value;
});
...
guardColor.addEventListener('input', () => {
   ...
   document.documentElement.style.setProperty('--accent', guardColor.value);
   document.querySelectorAll('.swatch').forEach(button => {
      button.classList.toggle(
         'active',
         button.dataset.color.toLowerCase() === guardColor.value.toLowerCase()
      );
   });
   render();
});
...
document.getElementById('swatches').addEventListener('click', event => {
   ...
   guardColor.dispatchEvent(new Event('input'));
});
